...
const std::vector<double> Histogram::GetSpeedsOfParticlesList(const std::vector<Particle>& particles) const {
...
void Histogram::CountParticlesInEachBar() {
  ...
  // Counts number of particles in speed range and adds the count to the bars_ list
  for (size_t bar = 0; bar < bars_.size(); bar++) {
    int particle_count = 0;
    for (size_t particle_speed = 0; particle_speed < particles_speeds_.size(); particle_speed++) {
